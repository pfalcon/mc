...
edit_get_save_file_as (WEdit * edit)
{
...

    {
        quick_widget_t quick_widgets[] = {
            /* *INDENT-OFF* */
            QUICK_LABELED_INPUT (N_("Enter file name:"), input_label_above,  edit->filename_vpath->str, "save-as",
...
edit_save_confirm_cmd (WEdit * edit)
{
    ...
    if (edit_confirm_save)
    {
        ...
        f = g_strdup_printf (_("Confirm save file: \"%s\""), edit->filename_vpath->str);
...
edit_ok_to_exit (WEdit * edit)
{
    ...
    if (edit->filename_vpath != NULL)
        fname = g_strdup (edit->filename_vpath->str);
#ifdef ENABLE_NLS
    else
