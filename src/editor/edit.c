...
edit_load_file_fast (WEdit * edit, const vfs_path_t * filename_vpath)
{
    ...
    if (file == -1)
    {
        ...
        errmsg = g_strdup_printf (_("Cannot open %s for reading"), filename_vpath->str);
    ...
    if (!ret)
    {
        ...
        errmsg = g_strdup_printf (_("Error reading %s"), filename_vpath->str);
        edit_error_dialog (_("Error"), errmsg);
        g_free (errmsg);
    }
    mc_close (file);
...
edit_find_filter (const vfs_path_t * filename_vpath)
{
    ...
    l = strlen (filename_vpath->str);
    for (i = 0; i < G_N_ELEMENTS (all_filters); i++)
    {
        e = strlen (all_filters[i].extension);
        if (l > e)
            if (!strcmp (all_filters[i].extension, filename_vpath->str + l - e))
                return i;
    }
    return -1;
}
...
edit_get_filter (const vfs_path_t * filename_vpath)
{
    ...
    quoted_name = name_quote (filename_vpath->str, 0);
...
check_file_access (WEdit * edit, const vfs_path_t * filename_vpath, struct stat *st)
{
    ...
    {
        errmsg = g_strdup_printf (_("Cannot get size/permissions for %s"), filename_vpath->str);
        goto cleanup;
    }
    ...
    {
        errmsg = g_strdup_printf (_("\"%s\" is not a regular file"), filename_vpath->str);
        goto cleanup;
    }
    ...
    if (st->st_size >= SIZE_LIMIT)
        errmsg = g_strdup_printf (_("File \"%s\" is too large"), filename_vpath->str);
