    ...
    case B_TREE:
        {
            ...
            if ((temp_dir[0] == '\0') || ((temp_dir[0] == '.') && (temp_dir[1] == '\0')))
                temp_dir = g_strdup (current_panel->cwd_vpath->str);
            else
    ...
    default:
        {
            ...
            *pattern = g_strdup (in_name->buffer);
            ...
            if (s[0] == '.' && s[1] == '\0')
            {
                *start_dir = g_strdup (current_panel->cwd_vpath->str);
            ...
            {
                /* relative paths will be used in panelization */
                *start_dir = mc_build_filename (current_panel->cwd_vpath->str, s, (char *) NULL);
                *start_dir_len = (ssize_t) strlen (current_panel->cwd_vpath->str);
                g_free (s);
            }
            ...
            while (dirp == NULL)
            {
                ...
                while (TRUE)
                {
                    ...
                    /* handle absolute ignore dirs here */
                    {
                        ...
                        ok = find_ignore_dir_search (tmp_vpath->str);
                        if (!ok)
                            break;
                    }
                ...
                g_free (directory);
                directory = g_strdup (tmp_vpath->str);
