...
exec_extension_view (void *target, char *cmd, const vfs_path_t * filename_vpath, int start_line)
{
    ...
    if (target == NULL)
        ...
        mcview_load ((mcview_t *) target, cmd, filename_vpath->str, start_line);
                ...
                int start_line)
{
    ...
    else
    {
        ...
        cmd = g_strconcat ("/bin/sh ", script_vpath->str, (char *) NULL);
    }
            ...
            *q = '\0';
            ...
            else if (strncmp (p, "regex/", 6) == 0)
            {
                ...
                if (search != NULL)
                {
                    search->search_type = MC_SEARCH_T_REGEX;
                    search->is_case_sensitive = !case_insense;
                    found = mc_search_run (search, filename_vpath->str, 0, file_len, NULL);
                    mc_search_free (search);
                }
            }
            else if (strncmp (p, "directory/", 10) == 0)
            {
                if (S_ISDIR (mystat.st_mode)
                    && mc_search (p + 10, filename_vpath->str, MC_SEARCH_T_REGEX))
                    found = TRUE;
            }
            else if (strncmp (p, "shell/", 6) == 0)
            {
                ...
                {
                    if (cmp_func (p, filename_vpath->str + file_len - (q - p), q - p) == 0)
                        found = TRUE;
                }
                else
                {
                    if ((size_t) (q - p) == file_len
                        && cmp_func (p, filename_vpath->str, q - p) == 0)
                        found = TRUE;
                }
            }
