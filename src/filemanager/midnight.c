...
put_current_path (void)
{
    ...
    command_insert (cmdline, cwd_vpath->str, FALSE);
    if (cwd_vpath->str[strlen (cwd_vpath->str) - 1] != PATH_SEP)
...
put_prog_name (void)
{
    ...
    if (get_current_type () == view_tree)
    {
        ...
        tmp = g_strdup (selected_name->str);
    }
    else
...
done_mc (void)
{
    ...
    if ((current_panel != NULL) && (get_current_type () == view_listing))
        vfs_stamp_path (current_panel->cwd_vpath->str);

    if ((other_panel != NULL) && (get_other_type () == view_listing))
        vfs_stamp_path (other_panel->cwd_vpath->str);
}
...
do_nc (void)
{
    ...
    else
    {
        ...
        if (mc_args__last_wd_file != NULL && vfs_current_is_local ())
            last_wd_string = g_strdup (current_panel->cwd_vpath->str);
