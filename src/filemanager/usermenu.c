    ...
    for (; *p != '\n' && *p != '&' && *p != '|'; p++)
    {
        ...
        *p |= 0x20;
        ...
        case 'd':
            p = extract_arg (p, arg, sizeof (arg));
            *condition = panel != NULL
                && mc_search (arg, panel->cwd_vpath->str, search_type) ? 1 : 0;
            break;
        case 't':
            p = extract_arg (p, arg, sizeof (arg));
...
execute_menu_command (WEdit * edit_widget, const char *commands, gboolean show_prompt)
{
    ...
    if (run_view)
    {
        mcview_viewer (file_name_vpath->str, NULL, 0);
    ...
    else
    {
        ...
        cmd = g_strconcat ("/bin/sh ", file_name_vpath->str, (char *) NULL);
        if (!show_prompt)
        {
            if (system (cmd) == -1)
    ...
    case 'd':
        {
            ...
            if (panel)
                cwd = g_strdup (panel->cwd_vpath->str);
            else
