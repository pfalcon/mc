...
erase_file (FileOpTotalContext * tctx, FileOpContext * ctx, const vfs_path_t * vpath)
{
    ...
    file_progress_show_deleting (ctx, vpath->str);
    if (check_progress_buttons (ctx) == FILE_ABORT)
        return FILE_ABORT;
...
panel_get_file (WPanel * panel)
{
    if (get_current_type () == view_tree)
    {
        ...
        return g_strdup (selected_name->str);
    }
    ...
    if (ctx->erase_at_end)
    {
        for (; erase_list != NULL && return_status != FILE_ABORT;)
        {
            ...
            if (S_ISDIR (lp->st_mode))
                ...
                return_status = erase_dir_iff_empty (ctx, lp->src_vpath->str);
            else
...
erase_dir (FileOpTotalContext * tctx, FileOpContext * ctx, const vfs_path_t * s_vpath)
{
    ...
    file_progress_show_deleting (ctx, s_vpath->str);
    return FILE_ABORT;
    ...
    if (error == 0)
    {                           /* not empty */
        error = query_recursive (ctx, s_vpath->str);
        if (error == FILE_CONT)
            error = recursive_erase (tctx, ctx, s_vpath->str);
        return error;
    }

    while (my_rmdir (s_vpath->str) == -1 && !ctx->skip_all)
    {
        error = file_error (_("Cannot remove directory \"%s\"\n%s"), s_vpath->str);
        if (error != FILE_RETRY)
            return error;
                            ...
                            uintmax_t total_size)
{
    ...
    g_snprintf (buffer, sizeof (buffer), _("%s\nDirectories: %zd, total size: %s"),
                str_trunc (dirname_vpath->str, WIDGET (this->dlg)->cols - 6), dir_count,
...
#define source_with_path source
    ...
    if (operation != OP_DELETE)
    {
        ...
        if (force_single)
            ...
            tmp_dest_dir = g_strdup (other_panel->cwd_vpath->str);
        else
            tmp_dest_dir = g_strdup (panel->cwd_vpath->str);
        /*
    ...
    if (do_bg)
    {
        ...
        v = do_background (ctx,
                           g_strconcat (op_names[operation], ": ", panel->cwd_vpath->str,
                                        (char *) NULL));
        if (v == -1)
    ...
    if ((vfs_path_tokens_count (panel->cwd_vpath) != 0)
        ...
        save_cwd = g_strdup (panel->cwd_vpath->str);
...
#endif /* WITH_FULL_PATHS */
        if (panel_operate_init_totals (operation, panel, source_with_vpath->str, ctx, dialog_type)
            == FILE_CONT)
        {
            if (operation == OP_DELETE)
            {
            ...
            {
                temp = transform_source (ctx, source_with_vpath->str);
                if (temp == NULL)
                ...
                else
                {
                    ...
                    case OP_COPY:
                        ...
                        if (S_ISDIR (src_stat.st_mode))
                            value = copy_dir_dir (tctx, ctx, source_with_vpath->str, dest,
                                                  TRUE, FALSE, FALSE, NULL);
                        else
                            value = copy_file_file (tctx, ctx, source_with_vpath->str, dest);
                        break;

                    case OP_MOVE:
                        if (S_ISDIR (src_stat.st_mode))
                            value = move_dir_dir (tctx, ctx, source_with_vpath->str, dest);
                        else
                            value = move_file_file (tctx, ctx, source_with_vpath->str, dest);
                        break;
            ...
            for (i = 0; i < panel->count; i++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        g_free (temp);
                        g_free (repl_dest);
                        source_with_path_str = strutils_shell_unescape (source_with_vpath->str);
                        temp3 = temp2;
