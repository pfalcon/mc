...
show_free_space (WPanel * panel)
{
    ...
    if (old_cwd == NULL || strcmp (old_cwd, panel->cwd_vpath->str) != 0)
    {
        char rpath[PATH_MAX];

        init_my_statfs ();
        g_free (old_cwd);
        old_cwd = g_strdup (panel->cwd_vpath->str);
...
goto_parent_dir (WPanel * panel)
{
    ...
    else
    {
        ...
        if (g_path_is_absolute (fname))
            ...
            fname = mc_build_filename (panelized_panel.root_vpath->str, fname, (char *) NULL);
...
_do_panel_cd (WPanel * panel, const vfs_path_t * new_dir_vpath, enum cd_enum cd_type)
{
    ...
    vfs_release_path (olddir_vpath->str);
    ...
    try_to_select (panel, get_parent_dir_name (panel->cwd_vpath->str, olddir_vpath->str));
...
panel_reload (WPanel * panel)
{
    ...
    ok = (panels_options.fast_reload && stat (panel->cwd_vpath->str, &current_stat) == 0
    ...
    do
    {
        ...
        if (panel->cwd_vpath->str[0] == PATH_SEP && panel->cwd_vpath->str[1] == '\0')
        {
            panel_clean_dir (panel);
            panel->count = set_zero_dir (&panel->dir) ? 1 : 0;
            return;
        }
        last_slash = strrchr (panel->cwd_vpath->str, PATH_SEP);
        vfs_path_free (panel->cwd_vpath);
        if (last_slash == NULL || last_slash == panel->cwd_vpath->str)
