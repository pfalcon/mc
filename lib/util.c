                    ...
                    GArray ** bookmarks)
{
    ...
    *offset = 0;
    ...
    while (fgets (buf, sizeof (buf), f) != NULL)
    {
        ...
        if (strncmp (buf, filename_vpath->str, len) != 0)
            continue;
                    ...
                    GArray * bookmarks)
{
    ...
    {
        if (fprintf
            (f, "%s %ld;%ld;%" PRIuMAX, filename_vpath->str, line, column, (uintmax_t) offset) < 0)
    ...
    {
        if (buf[len] == ' ' && strncmp (buf, filename_vpath->str, len) == 0
